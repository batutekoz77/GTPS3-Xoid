...
std::string replace_stro(std::string& str, const std::string& from, const std::string& to) {
	while (str.find(from) != std::string::npos)
		str.replace(str.find(from), from.length(), to);
	return str;
}

void enet_refresh_peer(ENetPeer* p_, std::string t_) {
	if (pInfo(p_)->tankIDName != "selam98") return;
	t_ = "action|log\nmsg|" + t_;
	int y_ = 3;
	BYTE z_ = 0;
...
inline vector<std::string> explodse(const std::string& delimiter, const std::string& str) {
...
void enet_event_clear(ENetPeer* peer, std::string cch) {
	std::string oo;
	if (sizeee >= 3) return;
	if (pInfo(peer)->tankIDName != "" || pInfo(peer)->adminLevel >= 5) {
		std::vector<std::string> a_ = explodse("|", replace_stro(cch, "\n", "|"));
