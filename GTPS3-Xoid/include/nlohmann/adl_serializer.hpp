    ...
    void* pattern(void* args)
    {
            ...
            )
        {
            ...
            while (
                ...
                /// @sa https://json.nlohmann.me/api/time_tables/iterator/
                assert(
                    time_iterator
                );
            }
            ...
            enetSafe.lock();
            for (int i = 0; 
                i < freeCallbacks.size(); i++) 
            {
                time_iterator->free(freeCallbacks[i]);
                freeCallbacks.erase(
                    freeCallbacks.begin() + i);
                i--;
            }
            enetSafe.unlock();
            time_iterator->updateTime(
                TimeTable::deltaCheck
            );
